...
### Multiple Layers
We can now repeat this process for the next layers, until we reach the last layer, the output layer. The final values in the output layer determine what the neural net "thinks" about its input. In our case of recommending YouTube videos, we would a neuron for every video, each with an activation that represents how likely the user is going to prefer the corresponding video. This process is essentially a repeated sequence of a linear transformation, a translation, then a nonlinear normalization function. Here's the full formula for the activation of each neuron:  
...
You might be wondering why we use multiple layers. The idea behind the neural network is that each layer should recognize some pattern, maybe a category of video type, then recognize compounded patterns in later layers, maybe a personality that likes to watch certain video categories. Using nonlinear normalization functions on each layer allows the neural net to focus on these patterns more. I say in theory because the patterns a neural net actually learns is very hard to deduce. There are actually many ways a machine can learn to recognize the number 4, most of which are not recognizable to us.
## Learning
...
def createTrie(tokens: list[str]) -> Trie:
    root = Trie(-1)
